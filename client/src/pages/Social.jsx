...
export default function Social() {
    ...
    useEffect(() => {
        ...
        const getMessages = async () => {
            try {
                ...
                const res = await axios.get(`http://localhost:5000/api/messages/${targetId}`);
    ...
    const sendRequest = async (targetId) => {
        try {
            await axios.post('http://localhost:5000/api/friends/request', { receiver_id: targetId });
    ...
    const acceptRequest = async (senderId) => {
        try {
            await axios.put('http://localhost:5000/api/friends/accept', { sender_id: senderId });
            setFriendsRefresh(prev => prev + 1);
    ...
    const cancelConnection = async (targetUserId) => {
        ...
        try {
            await axios.delete(`http://localhost:5000/api/friends/${targetUserId}`);
            setFriendsRefresh(prev => prev + 1);
